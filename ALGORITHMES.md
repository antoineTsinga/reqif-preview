...
```
 ...
 ReqIfDocument (modèle typé, acyclique — src/types.ts)
        ...
        └──► render.ts ───────────────────────────────────────────────────┐
                │                                                          │
                ├─ createAttachmentLookup() : pré-résout tous les          │
...
Chaque flèche correspond à une fonction pure, ce qui rend chaque étape testable isolément — c'est ainsi que sont structurés les tests dans `test/`.

**Une seule exception, et elle est explicite** : le rendu de l'arbre transporte un `Set<string>` des identifiants d'ancre déjà émis, afin qu'un `SpecObject` figurant à plusieurs endroits ne produise pas plusieurs fois le même `id` (section 8). Il est passé en paramètre comme n'importe quelle autre donnée — jamais en variable de module — et `renderPackageToHtml` en crée un par paquet. C'est la différence entre un état partagé *visible dans les signatures* et un état caché : le premier reste testable, le second non.
...
- **`layout: "tabs"`** (`tabs.ts`) : les onglets sont de vrais liens, et **le fragment d'URL décide du panneau affiché**. Trois règles statiques suffisent — `.reqif-tab-panel:target` (l'onglet a été cliqué : la cible *est* le panneau), `.reqif-tab-panel:has(:target)` (un lien profond a atterri dedans : le panneau est un *ancêtre* de la cible), et `.reqif-tabs:not(:has(:target)) > .reqif-tab-panel:first-of-type` (rien de ciblé dans ce groupe : le premier panneau). Seul le surlignage de l'onglet actif demande encore une règle par onglet. Les identifiants de panneaux dérivent des identifiants ReqIF, jamais de la position : un lien partagé survit à l'insertion d'un document avant lui.
  - **Pourquoi pas les cases radio cachées**, technique pourtant classique : l'état vivait alors dans le DOM, ce qui isolait bien l'aperçu de sa page hôte, mais rendait **toute ancre profonde impossible** — un lien de relation vers un objet d'un autre onglet faisait défiler vers un élément encore en `display:none`. Avec `:target`, un seul mécanisme porte l'état, donc rien ne peut entrer en conflit, et une ancre profonde ouvre d'elle-même tous les panneaux menant à sa cible, **à tous les niveaux d'imbrication à la fois** (documents × spécifications). Deux effets de bord de cet abandon : la contrainte structurelle du combinateur `~` disparaît (les panneaux n'ont plus à être frères des `<input>`), et `tabs.ts` **n'a plus d'état** — le compteur de module et son suffixe aléatoire étaient là pour éviter les collisions d'identifiants de groupe, rôle que les identifiants ReqIF remplissent désormais.
  - **Contreparties assumées** : l'état des onglets vit dans le fragment d'URL de la page hôte, donc une application qui route sur le hash verra sa route changer, et deux aperçus sur une même page ne peuvent plus retenir deux sélections indépendantes. Le mécanisme exige `:has()` (disponible partout depuis décembre 2023).
...
- **Mode lecture** : n'ajoute aucune nouvelle structure de données — il fait simplement des branches conditionnelles (`options.readingMode ? "" : ...`) sur des fragments déjà calculés (badge ID, puce créé/modifié, panneau technique), et bascule le tag de titre d'un texte en gras vers `<h3>`–`<h6>` (profondeur + 2, plafonnée à 6) pour une meilleure structure sémantique/visuelle. Une classe `reqif-reading-mode` posée sur le conteneur racine active les surcharges CSS correspondantes (suppression des bordures de carte, etc.) sans dupliquer le HTML.
  - **Piège évité** : les tailles par défaut du navigateur pour `<h3>`–`<h6>` sont en `em`, donc relatives à la taille de police *héritée*, pas à la racine. Comme chaque niveau de titre est imbriqué dans le conteneur du niveau précédent, ces tailles se **multiplient en cascade** à chaque profondeur (0,83 × 0,83 × 0,83...), ce qui fait rétrécir le texte de façon exponentielle sur une arborescence profonde — exactement le symptôme observé ("plus on descend, plus c'est petit"). La correction fixe une taille en `rem` (toujours relative à la racine, jamais à un ancêtre imbriqué) pour chacun des 4 niveaux, avec un plancher à `0.95rem` qui ne bouge plus au-delà de `<h6>` au lieu de continuer à rétrécir indéfiniment.
- **Placeholders vides volontaires (`suppressEmptyPlaceholdersForChapters`, `isTitleless`, `isContentless`)** : trois notions sont calculées et combinées indépendamment pour un même nœud. `isChapter` (l'objet correspond-il à `chapterNumberAttributes` ? toujours calculé et exposé sur le contexte des rendus personnalisés, indépendamment de tout autre réglage) sert de raccourci pratique (`chapterShortcut = suppressEmptyPlaceholdersForChapters && isChapter`). Mais le besoin réel est plus large qu'une simple notion de "chapitre" — un objet peut être structurellement censé n'avoir aucun titre (un paragraphe d'information) sans pour autant être censé n'avoir aucun contenu, et inversement. D'où deux prédicats généraux et complètement indépendants, fournis par l'appelant : `isTitleless(obj, specType, index)` et `isContentless(obj, specType, index)`, chacun composé en OU avec le raccourci chapitre (`suppressTitle = chapterShortcut || isTitleless(...)`, `suppressContent = chapterShortcut || isContentless(...)`) — jamais l'inverse (un prédicat ne peut pas RÉACTIVER un placeholder que l'autre mécanisme aurait déjà supprimé). Ces prédicats ne s'appliquent qu'au repli : un `LONG-NAME`/contenu réellement présent s'affiche toujours, quoi qu'ils renvoient — ils ne contrôlent que ce qui se passe quand il n'y a, structurellement, rien à afficher.
- **Liens entre exigences (`SpecRelation`)** : `ReqIfIndex` construit, dans la même passe O(n) que le reste (section 5), deux `Map<Identifier, SpecRelation[]>` supplémentaires — `outgoingRelations` et `incomingRelations` — indexées respectivement par `sourceRef` et `targetRef`. Au rendu, chaque `<details class="reqif-node">` reçoit un `id` stable dérivé de l'identifiant ReqIF de son objet (caractères hors `[A-Za-z0-9_-]` remplacés par `_`, voir `domId`), ce qui permet à un lien de relation d'utiliser une simple ancre `<a href="#reqif-obj-...">` vers l'objet lié **s'il est rendu dans la même page**. Un même `SpecObject` pouvant légalement figurer à plusieurs endroits de l'arbre, seule **la première occurrence porte l'`id`** (un `Set` transmis dans toute la récursion, comme l'est `basePath`) : c'est ce que fait déjà un navigateur en résolvant un fragment, et les doublons produisaient un HTML invalide sans rendre les autres occurrences atteignables. Chaque doublon émet un `duplicate-dom-id` — aucune logique de recherche DOM à l'exécution, juste une convention de nommage déterministe partagée entre l'émission de l'`id` et celle du lien.
