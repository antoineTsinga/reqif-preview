...
- **Numérotation de chapitre** : un tableau `path: number[]` est propagé à travers la récursion de `renderHierarchyNode`, chaque niveau ajoutant l'index 1-based du frère courant (`[...path, i + 1]`) — exactement le même genre de paramètre accumulé par récursion qu'utilise déjà la construction de la hiérarchie (section 3). La numérotation est remise à `[]` à chaque nouvelle `Specification`.
